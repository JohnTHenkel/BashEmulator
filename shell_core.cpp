...
Shell::Shell() {
  ...
  //Read in history from previous session
  string line;
  ...
  if(history_in.is_open()){
	...
	cerr << "No history file detected"<<endl;
  }
...
int Shell::execute_line(char* line) {
  ...
  //add command to history file to be preserved
  ofstream history_out;
  history_out.open("history.txt", ios::out | ios::app | ios::ate);
  history_out << line <<endl;
  history_out.close();
...
void Shell::alias_substitution(vector<string>& tokens) {
 	for (auto& kv: aliases){
		if(aliases.count(tokens.at(0))){
			tokens.at(0)=kv.second;
		}	
	}
